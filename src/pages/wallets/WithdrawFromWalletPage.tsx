...
export default function WithdrawFromWalletPage() {
  ...
  const totalToDebit = parsedAmount + WITHDRAWAL_FEE
  const exceedsAvailable = parsedAmount > availableAmount
  const belowMinimum =
  ...
  const handleOpenPinModal = () => {
    ...
    if (!isFormValid) {
      if (parsedAmount <= 0) setError('Enter an amount to withdraw.')
  ...
  const handlePinVerification = async (_pin: string) => {
    ...
    try {
      ...
      // Simulated failure hook — flip to true to test the error path.
    ...
    <AppLayout>
      <div className="py-5" style={{ color: themeColors.charcoal }}>
        ...
        {parsedAmount > 0 && (
          ...
          >
            ...
            <div className="flex items-center justify-between">
              ...
              >
                {formatCurrency(
                  Math.max(availableAmount - totalToDebit, 0)
                )}
              </span>
            </div>
          </div>
        ...
        >
          ...
          Withdraw {parsedAmount > 0 ? formatCurrency(parsedAmount) : ''}
        </button>
        ...
        <p
          ...
          style={{ color: themeColors.mid }}
        >
          Funds will be sent to your linked bank account. You'll be asked for
          your PIN to confirm.
        </p>
      </div>
